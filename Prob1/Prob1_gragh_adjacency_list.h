...
vertex* searchvertex(vertex* first, void* name) { // *(char*)name == *(char*)리턴vertex->nameptr 인 vertex를 찾는다 (first vertex부터 차례대로)
	if (!first)
		printf("searchvertex) g1->first가 %p네요\n", first);
	else {
...
gragh* deleteedge(gragh* g1, void* from, void* to) { // 11.25에러: B->outdeg와 D->indeg가 NULL이 아니기만 하면 B->D가 없어도 된다->for문 오작동->처리함
	...
	if (start == NULL || end == NULL)      
		...
		if (pDelE->next == NULL && pDelE->src != end) // for문 예외때문에 마지막 비교를 한 번 더 하도록 만듦
			...
			printf("start->outdeg 중 edge 주소 %p를 지우겠습니다\t(참고:&pDelE = %p)\n", pDelE, &pDelE);
			free(pDelE);
			pDelE = NULL;
		}
		...
		if (pDelE->next == NULL && pDelE->src != start) // for문 예외때문에 만듦
			...
			if (pPreE == NULL) // 첫번째를 지우는 case
				...
				pPreE->next = pDelE->next;
			printf("end->indeg 중 edge 주소 %p를 지우겠습니다\t(참고:&pDelE = %p)\n", pDelE,&pDelE);
			free(pDelE);
			pDelE = NULL;
		}
	}
...
gragh* deletevertex(gragh* g1, void* del) { //1.pDelV outdeg 및 반대 indeg 삭제 2.pDelV indeg 및 반대 outdeg 삭제 3.pDelV 삭제 전 이어주기 4.pDelV 삭제
	...
	if (!pDelV)
		...
		if(pDelV->outdeg==NULL&&pDelV->indeg==NULL)
			printf("(처음) %c->outdeg Addr = %p\t%c->indeg Addr = %p로 자신을 free할 준비가 이미 되어있네요\n", *(char*)pDelV->nameptr, pDelV->outdeg, *(char*)pDelV->nameptr, pDelV->indeg);
		else {
			printf("(처음) %c->outdeg Addr = %p\t%c->indeg Addr = %p로 자신을 free할 준비가 안됐습니다.\n", *(char*)pDelV->nameptr, pDelV->outdeg, *(char*)pDelV->nameptr, pDelV->indeg);
			eTemp = pDelV->outdeg; // 1.먼저 pDelV outdeg 처리
			printf("%c->outdeg들 및 반대 indeg까지 free하려고 합니다.\n", *(char*)pDelV->nameptr);
			/*while (eTemp) { // outdeg를 파고들다 보면 NULL을 만날 것이다
				g1 = deleteedge(g1, pDelV->nameptr, eTemp->src->nameptr); // 11.25에러: 의도대로라면 NULL을 만나야하는데 0xdddddddd를 만남->끝에서 부터 지우자(스택또는재귀의원리)
				eTemp = eTemp->next;
			}*/
			while (eTemp) {
				STemp[idx++] = eTemp;
			...
			printf("%c->indeg들 및 반대 outdeg까지 free하려고 합니다.\n", *(char*)pDelV->nameptr);
			idx = 0;
			while (eTemp) {
				STemp[idx++] = eTemp;
				eTemp = eTemp->next;
			...
			/*while (eTemp) { // indeg를 파고들다 보면 NULL을 만날 것이다
				g1 = deleteedge(g1, eTemp->src->nameptr, pDelV->nameptr);
				eTemp = eTemp->next;
			}*/
			/*//3. 삭제 전 이어주기
			...
			//4. V 삭제
			printf("(결과) %c->outdeg Addr = %p\t%c->indeg Addr = %p로 자신을 free할 준비가 되었습니다.\n", *(char*)pDelV->nameptr, pDelV->outdeg, *(char*)pDelV->nameptr, pDelV->indeg);
		}
		//3. 삭제 전 이어주기
		if (g1->first == pDelV) // case 1 : 첫 번째 V가 삭제 : 이유 : 이 경우는 아래 for문을 빠져나오지 못함
			g1->first = pDelV->next;
...
void deletegragh(gragh* g1) {
	...
	/*while (Temp) {
		g1 = deletevertex(g1, Temp->nameptr); //11.25에러:Temp=0xdddddddd -> 끝에서부터 지우자 ->
		Temp = Temp->next;
	}*/
	free(g1);
}
...
gragh* addedge(gragh* g1, void* from, void* to) { // 11.25에러: A->B가 이미 있는데 A->B를하면 추가되는 현상 -> 처리함
	...
	if (start == NULL || end == NULL)
		printf("그래프 안에 찾는 값(from, to)이 안 들어있네요..?\n");
	else {
		etemp = start->outdeg;
		while (etemp) {
			if (*(char*)etemp->src->nameptr == *(char*)end->nameptr) {
				printf("%c->outdeg 중에 이미 %c가 있다고 합니다.\n", *(char*)start->nameptr, *(char*)end->nameptr);
				return g1;
			}
			etemp = etemp->next;
		}
		//11.25 start->outdeg와 end->indeg는 동시에 만들어지니까 start->outdeg case만 확인해봐도 될 것 같다.
...
void printarc(vertex* ver) { // 11.25에러: g1->first가 NULL일 때 걸러야하는데 recursion이라..-> if(ver==NULL)을 추가했다
	edge* eTemp = NULL; // 11.25에러:A,B vertex를 만들고 A를 지우니까 ver->nameptr=0xddddddd에 접근에러//이유:g1->first가 B로 바뀌지 않았음->반정도 처리
	if (ver == NULL)
		printf("printarc) vertex가 %p입니다\n", ver);
	else {
